...
=======
/* --- ROUTES: GAME LOGIC --- */
>>>>>>> parent of e738614 (Refactor random content API to improve selection logic)
...
<<<<<<< HEAD
        ...
        const getRandomQuestion = async (type) => {
            return new Promise((resolve, reject) => {
                // For character and banner types, ensure media_path is not null
                const mediaCheck = (type === 'character' || type === 'banner') 
                    ? 'AND q.media_path IS NOT NULL' 
                    : '';
                
                const sql = `
                    SELECT q.*, f.title as franchise_title, f.category as franchise_category, f.id as franchise_id
                    FROM questions q
                    JOIN franchises f ON q.franchise_id = f.id
                    WHERE q.type = ? AND f.category = ? ${mediaCheck}
                    ORDER BY RANDOM()
                    LIMIT 1
                `;
        ...
        // Try to get all three types with retries
        const maxRetries = 10;
        
        for (let i = 0; i < maxRetries && !quoteQ; i++) {
            quoteQ = await getRandomQuestion('quote');
        }
        
        for (let i = 0; i < maxRetries && !charQ; i++) {
            charQ = await getRandomQuestion('character');
        }
        
        for (let i = 0; i < maxRetries && !bannerQ; i++) {
        ...
        if (!quoteQ || !charQ || !bannerQ) {
            const missing = [];
            if (!quoteQ) missing.push('quotes');
            if (!charQ) missing.push('characters');
            if (!bannerQ) missing.push('banners');
            
            return res.status(404).json({ 
                error: `Not enough content available for this category. Missing: ${missing.join(', ')}` 
            });
=======
...
>>>>>>> parent of 592faac (Refactor random content API to decouple question selection)
});
